...
import { View, ScrollView, SafeAreaView, KeyboardAvoidingView, StatusBar, ActivityIndicator, Alert, Platform } from 'react-native';
...
const App = () => {
  ...
  useEffect(() => {
    const checkForUpdates = async () => {
      if (Platform.OS !== 'web' && !__DEV__) { // Check if running in a non-development build
        try {
  ...
  const generateSection = (title, items) => `
    ...
    <ul class="item-list">
      ${items.split(',').map(item => {
        ...
        return name ? `<li class="item"><span class="itemName">${name}</span><span class="itemQuantity">${quantity || 'N/A'}</span></li>` : '';
      }).join('')}
    </ul>
  `;
